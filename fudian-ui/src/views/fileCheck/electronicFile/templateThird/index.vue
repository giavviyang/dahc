<template>
  <div class="app-container">
      ...
      <el-tab-pane label="已领取" name="alreadyReceived" :disabled="tabDisabled">
        ...
        <div class="checkBox">
          ...
          <PicProcess class="filePic" :imgList="imgList" :imgDes="imgDes" :currentPage='currentPage'
                      :isCurrentPage="isCurrentPage" :isCurrentPageItem="isCurrentPageItem" :currentFile="currentFile"
                      ...
                      @modifyThePageNumber="modifyThePageNumber"></PicProcess>
            ...
            <div class="fileContentForm">
              <el-form ref="form" :model="fileForm" label-width="80px" size="mini">
                <el-form-item v-for="(item,index) in  fileColumn" :label="item.label" :key="index">
                  <el-input v-model="fileForm[item.prop]" type="textarea"
    ...
    <el-dialog title="重置页号"
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" size="mini"
                   @click="modifyThePageNumberSave('modifyThePageNumberForm')">保存</el-button>
    ...
    <SimpleUploader ref="simpleUploader" :projectId="receptionProcedure.projectId" :procedureId="receptionProcedureId"
                    ...
                    :checkStatus="isImpeach" :timerSwitch="timerSwitch" :keyName="keyName"
                    @queryCaseLevelArchivesDossierLevelTemplateTwo="queryCaseLevelArchivesDossierLevelTemplateTwo" @onFileSuccess="onFileSuccess"
                    @closeSimpleUploader="closeSimpleUploader" :caseFileNumber="caseFileNumber"
                    @getInformation="getInformation"
                    :pid="pid"></SimpleUploader>
  </div>
</template>
...
import {
  ...
  download,
  imageReplacement,
  thePictureMovesUpAndDown, imagePlusPageTwo, modifyTheNumberOfPages, associateTheCurrentOperation
...
import {
  ...
  obtainCaseDataAttr, caseFileNumberAttr
} from "@/api/business/archiveType";
import {modifyTheSort, updateFileData, increaseCases, getInformation} from "@/api/fileData/fileData";
import {getData} from "@/api/dahc/sys/dictData";
import imgUpload from "@/views/upload/imgUpload";
import imgsUpload from "@/views/upload/imgsUpload";
  ...
  methods: {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.modifyThePageNumberForm.caseFileNumber = this.caseData[0]["attr" + this.queryParams.attrOrder], //案卷名称
            ...
            this.modifyThePageNumberForm.checkStatus = this.isImpeach /*是否是存疑页面*/
    ...
    pagingDown() {
      this.$confirm('此操作将删除文件：' + this.imgList[this.currentPage].keyName + '下图片名称为：' + this.imgList[this.currentPage].fileName + '的数据, 是否继续?', '提示', {
        ...
        const arr = {
          ...
          checkStatus: this.isImpeach /*是否是存疑页面*/
        }
      ...
      if (this.uploadImgsFiles.length == 0) {
        ...
        if (this.imgList.length > 0) {
          const arr = {
            ...
            keyName: this.imgList[this.currentPage].keyName, //案卷名称
            procedureId: this.receptionProcedureId,
            buttonName: "图片加页",
    ...
    uploadAdd() {
      if (this.uploadFiles.length == 0) {
        ...
        formData.append('dto', JSON.stringify(arr))
        console.log(this.uploadFiles, "更改")
    ...
    imgMoveUp() {
      if (this.currentPage == 0) {
        ...
        const arr = {
          ...
          pid: this.fileForm.attrId,//文件id
          caseFileNumber: this.caseData[0]["attr" + this.queryParams.attrOrder], //案卷名称
          keyName: this.fileForm["attr" + this.caseFileAttrOrder], //文件序号
          procedureId: this.receptionProcedureId, //工序名称,
          buttonName: "图片上移",
          projectId: this.receptionProcedure.projectId,//项目Id
          checkStatus: this.isImpeach /*是否是存疑页面*/
    ...
    nextFile(val) {
      ...
      this.isDisabled = false;
      this.fileForm = this.fileData[this.currentFile];
      // this.handleRowClick( {row:this.fileForm})
      this.imgList = [];
      this.reset();
      /*查询图片*/
      this.getPucter();
    ...
    handleTabClick(tab, event) {
      ...
      if (this.queryParams.activeName == 'noReceived') {
        this.handleQuery("0");
        document.onkeydown = null;
      } else if (this.queryParams.activeName === 'alreadyReceived') {
        this.handleQuery("2");
        this.keyDown(0);
      }
    },
    ...
    handleBack() {
      ...
      //     item.rollId = this.caseData[0].filesName; /*档案名称*/
      //     item.isInspectAccomplish = 1 /*核查结果状态 1-未完成 0-完成*/
      //     item.fileId = this.caseData[0].filesId /*档案主键id*/
      //     item.recordProcedureFilesId = this.caseData[0].id /*当前核查结果记录的主键id*/
      //   }
      // );
      // /*保存核查数据*/
      ...
      mergeFile(file).then(responseData => {
        ...
        var interval = setInterval(() => {
          if (this.timerSwitch == '完成但是失败') {
            ...
            console.log("停止2")
            this.timerSwitch = '';
            console.log(this.timerSwitch,"停止后")
          } else {
