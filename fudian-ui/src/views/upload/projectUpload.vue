<template>
  ...
  <div>
    <el-dialog
      ...
      :before-close="closeSimpleUploader">
    <uploader
      ref="uploader"
      :options="options"
      :autoStart=false
      ...
      <uploader-drop>
        <div>
          <uploader-btn id="global-uploader-btn" :attrs="attrs" ref="uploadBtn">选择文件<i
            class="el-icon-upload el-icon--right"></i></uploader-btn>只能上传zip文件
...
<script>
  ...
  import {getInformation} from "@/api/fileData/fileData";
  ...
  export default {
    ...
    methods: {
      ...
      closeSimpleUploader() {
        let show = false;
        this.$emit("closeSimpleUploader", show);
      },
      onFileAdded(file) {
      ...
      */
      ...
      getInformation(val) {
        ...
        getInformation(val).then(res => {
          ...
          if (res.data == '完成但是失败') {
            ...
            this.$message({message: res.msg, type: 'success', showClose: true,duration: 0,onClose: ()=>{
                /*点击关闭执行事件*/
      ...
      queryCaseLevelArchivesDossierLevelTemplateTwo() {
        console.log("刷新文件表格，点击x")
        this.$emit("queryCaseLevelArchivesDossierLevelTemplateTwo");
      },
      /**
       * 计算md5，实现断点续传及秒传
       * @param file
